...
$perCtr_query = $conn->query("SELECT personnel_id FROM personnels WHERE separation_date='' OR separation_date='  /  /    '");
$perCtr_all=$perCtr_query->rowCount();

$perCtrMale_query = $conn->query("SELECT personnel_id FROM personnels WHERE sex='Male' AND (separation_date='' OR separation_date='  /  /    ')");
$perCtrM_all=$perCtrMale_query->rowCount();

$perCtrFemale_query = $conn->query("SELECT personnel_id FROM personnels WHERE sex='Female' AND (separation_date='' OR separation_date='  /  /    ')");
