...
if($_SERVER['REQUEST_METHOD'] === 'POST' && (isset($_POST['save_personnel_income']) || isset($_POST['personnel_id']))) {
    ...
    try {
        ...
        if (empty($personnel_id)) {
            ...
            header('Location: list_personnel.php?dept=' . urlencode($dept) . '&errorMsg=' . urlencode('Personnel ID is required'));
            exit();
        }
        ...
        if (!$table_exists) {
            ob_end_clean(); // Clear output buffer
            header('Location: list_personnel_income.php?dept=' . urlencode($dept) . '&personnel_id=' . urlencode($personnel_id) . '&errorMsg=' . urlencode('Database table not created yet'));
            exit();
        }
        ...
        $redirect_url = 'list_personnel_income.php?dept=' . urlencode($dept) . 
                       ...
                       '&successMsg=' . urlencode('Personnel income has been updated successfully.');
        ...
        $redirect_url = 'list_personnel_income.php?dept=' . urlencode($dept) . 
                       '&personnel_id=' . urlencode($personnel_id) . 
                       '&errorMsg=' . urlencode('Database error occurred');
...
if ($personnel_id && $dept) {
    header('Location: list_personnel_income.php?dept=' . urlencode($dept) . '&personnel_id=' . urlencode($personnel_id) . '&errorMsg=' . urlencode('Invalid form submission'));
} else {
    header('Location: list_personnel.php?errorMsg=' . urlencode('Invalid form submission'));
}
exit();
?>
