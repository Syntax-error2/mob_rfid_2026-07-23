...
<div class="col-lg-12">
    ...
    <?php
    $LV_query = $conn->query("select * FROM personnel_logs WHERE logDate='$_GET[dateFrom]' ORDER BY log_id DESC") or die(mysql_error());
    while($LV_row=$LV_query->fetch()){
    
    $printAll_Data_query = $conn->query("select * FROM personnels WHERE RFTag_id='$LV_row[RFTag_id]'") or die(mysql_error());
