...
$printAll_Data_query = $conn->query("select * FROM personnels WHERE do_id='$_GET[do_id]' AND (separation_date='' OR separation_date='  /  /    ') ORDER BY lname, fname ASC") or die(mysql_error());
while($printALL_row=$printAll_Data_query->fetch()){


$studData_query = $conn->query("select * FROM personnels WHERE RFTag_id='$printALL_row[RFTag_id]' AND do_id='$_GET[do_id]'") or die(mysql_error());
...
<?php
  ...
  <td <?php if($SC_query3->rowCount()>0){?> rowspan="2" <?php } ?> >
    ...
    $studLogs_remarks_query = $conn->query("select * FROM personnel_logs WHERE RFTag_id='$RFTag_id' AND logDate='$logDateCtr' AND (remarks!='' AND remarks!='Updated' AND remarks!='Inserted' AND remarks!='24hrs')") or die(mysql_error());
    ...
    if($studLogs_AM_IN_row['late_status']==='on'){
        ...
        $sched_query = $conn->query("SELECT am_IN_co FROM time_schedules WHERE do_id='$studData_row[do_id]' AND shift_id='$studData_row[shift_id]'AND day='$dayName'") or die(mysql_error());
    ...
    if($studLogs_AM_OUT_row['late_status']==='on'){
        ...
        $sched_query = $conn->query("SELECT am_OUT FROM time_schedules WHERE do_id='$studData_row[do_id]' AND shift_id='$studData_row[shift_id]' AND day='$dayName'") or die(mysql_error());
        $sq_row=$sched_query->fetch();
    ...
    if($studLogs_PM_IN_row['late_status']==='on'){
        ...
        $sched_query = $conn->query("SELECT pm_IN_co FROM time_schedules WHERE do_id='$studData_row[do_id]' AND shift_id='$studData_row[shift_id]'AND day='$dayName'") or die(mysql_error());
        $sq_row=$sched_query->fetch();
    ...
    if($studLogs_PM_OUT_row['late_status']=="on"){
        ...
        $sched_query = $conn->query("SELECT pm_OUT FROM time_schedules WHERE do_id='$studData_row[do_id]' AND shift_id='$studData_row[shift_id]' AND day='$dayName'") or die(mysql_error());
        $sq_row=$sched_query->fetch();
...
Verified as to the prescribed office hours. <br /> 
  ...
  <?php
     ...
     $adminData_query = $conn->query("select do_id FROM personnels WHERE RFTag_id='$printALL_row[RFTag_id]'") or die(mysql_error());
     $adminData_row=$adminData_query->fetch();
    
    
    $dept_off_query = $conn->query("SELECT officeHead_id FROM dept_offices WHERE do_id='$adminData_row[do_id]'") or die(mysql_error());
    $do_row = $dept_off_query->fetch(); 
    
    
    $officeHead_query = $conn->query("SELECT lname, fname, mname, suffix FROM personnels WHERE personnel_id='$do_row[officeHead_id]'") or die(mysql_error());
